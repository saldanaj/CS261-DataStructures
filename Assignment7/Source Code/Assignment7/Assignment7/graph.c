...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    // ensure the variable isVisited for each of the
    // nodes/vertices in the graph is cleared (set to 0)
    clearVisited(graph);
    ...
    // create the reachable stack and push onto the stack
    // the vertices we reached throughout the DFS algorithm
    Deque *reachableStack  = dequeNew();
    assert(reachableStack != 0);
    
    // add the source vertex to the reachable stack
    // and we'll pop it after to see what we can reach from there
    dequePushBack(reachableStack, source);
    
    while(!dequeIsEmpty(reachableStack));
    {
        // assign current the vertex at the "top" of the
        // stack and then remove it from the stack
        Vertex * current = dequeBack(reachableStack);
        
        dequePopBack(reachableStack);
        
        // assign the value of 1 to the vertex to it's
        // "isVisited" variable to reflect the node/vertex is
        ...
        for(int a = 0; a < current->numNeighbors; a++)
        {
            if(current->neighbors[a]->isVisited == 0)
            {
                dequePushBack(reachableStack, current->neighbors[a]);
//                current->neighbors[a]->isVisited = 1;
            }
            
        }
    }
    
    // delete the reachable stack
    
    dequeDelete(reachableStack);
    
    // return 0 because we were unable to reach the destination
    return 0;
}
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    // create the reachable stack and push onto the stack
    // the vertices we reached throughout the DFS algorithm
    Deque *reachableQueue  = dequeNew();
    assert(reachableQueue != 0);
    
    // add the source vertex to the reachable stack
    // and we'll pop it after to see what we can reach from there
    dequePushBack(reachableQueue, source);
    
    
    
    while (!dequeIsEmpty(reachableQueue))
    {
        Vertex *current = dequeFront(reachableQueue);
        
        dequePopFront(reachableQueue);
        ...
        for (int a = 0; a < current->numNeighbors; a++)
        {
            if (current->neighbors[a]->isVisited == 0)
            {
                dequePushBack(reachableQueue, current->neighbors[a]);
//                current->neighbors[a]->isVisited = 1;
    ...
    // delete the reachable queue
    dequeDelete(reachableQueue);
