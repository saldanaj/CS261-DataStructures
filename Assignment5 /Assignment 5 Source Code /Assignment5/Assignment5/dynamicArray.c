...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if(rightChild < last)
    {
        smallestChild = _smallerIndexHeap(heap, leftChild, rightChild, compare);
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
    ...
    // starting with the first non leaf node and percolating from there
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
    ...
    // up to, but not including the last element in the array
    int last = dySize(heap) - 1;
    
    dyPut(heap, dyGet(heap, last), 0);
    dyRemoveAt(heap, last);
    
    // now adjust the tree recursively
 ...
 * @param heap array with elements
 * @param first index we want to compare 
 * @param second index we want to compare 
 * @parama compare pointer to compare function 
 * @return return index of smallest element
 */

int _smallerIndexHeap(DynamicArray *heap, int i, int j, compareFunction compare)
{
    // function written as described in worksheet 33: heaps and priority queues
    
    assert(i < dySize(heap));
    assert(j < dySize(heap));
